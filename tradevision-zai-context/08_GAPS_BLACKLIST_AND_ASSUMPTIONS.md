...
- NEWS-FEED-1 (2026-08-17): `apps/news_feed` is no longer an empty scaffold. It now has a real provider-backed ingestion pipeline (Marketaux, ADR-029), a `news_feed_newsitem` store, `GET /api/v1/news/` (paginated, `?symbol=` filter) + `GET /api/v1/news/<uuid>/`, a Celery beat task, and it feeds `IntelligencePacket.news_context`. Shape verified via the API integration tests (`apps/news_feed/tests/test_api.py`). Sentiment contract: `sentiment_score` is a string or `null`; `sentiment_label` is always `null` (ADR-029 — no provider supplies a label; never hand-rolled → frontend renders `--`). Empty feed → `{count:0,next:null,previous:null,results:[]}`.
